...
def print_status(game):
    ...
    if(not ball_out):
        map[game.ball.pos.y][game.ball.pos.x] = colors.bcolors.OKGREEN + "o" + colors.bcolors.ENDC
    ...
    for paddle in game.paddles:
        ...
        for i in range(-aux, aux+1):
            ...
            map[paddle.pos.y][paddle.pos.x + i] = colors.bcolors.OKBLUE + "□" + colors.bcolors.ENDC
...
def draw(dis, game):
    ...
    ball_out = game.ball.pos.y < 0 or game.ball.pos.y >= game.map.h or game.ball.pos.x < 0 or game.ball.pos.x >= game.map.w
    if(not ball_out):
        pygame.draw.circle(dis, colors.rgb.RED, (game.ball.pos.x*scale + scale/2, game.ball.pos.y*scale + scale/2), scale/2)

    for paddle in game.paddles:
        aux = math.floor(paddle.len/2)
        for i in range(-aux, aux+1):
            if(paddle.pos.x + i < 0 or paddle.pos.x + i >= game.map.w): continue
            pygame.draw.rect(dis, colors.rgb.BLUE, [(paddle.pos.x + i)*scale, paddle.pos.y*scale, scale, scale])
